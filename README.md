...
|  页面截图 | 该模块实现了页面截图功能 | windows/linux |  |
...
|  云主机秘钥泄露利用 | 该模块用于利用泄露的密钥，针对阿里云、腾讯云、华为云等云厂商的泄露密钥进行安全评估和漏洞利用 | windows/linux | 暂不对外开放 |
...
|  基于ICMP远程控制 | 该模块利用 ICMP 报文实现远程指令传输与数据回传，适用于隐蔽通信与受限网络环境 | windows/linux | 暂不对外开放 |
